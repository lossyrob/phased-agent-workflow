...
Treat `WorkflowContext.md` as the durable artifact-lifecycle source of truth. If it includes a `## Control State` section, preserve that section exactly and update only the lifecycle field. If control state is absent, treat the workflow as legacy best-effort mode.
