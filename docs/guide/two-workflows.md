...
- **Full human control** — Output actions come from available tools and require explicit authorization against the exact target, head/snapshot, action, and output resource
- **Hosted Azure DevOps context** — GET-only acquisition captures PR, diff, iteration, discussion, reviewer, policy, status, and build state independently from tool-driven output capability
