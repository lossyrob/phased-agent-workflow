...
This strategy is distinct from all other specialists because it doesn't analyze what the code *does* — it analyzes what the code *assumes*. Where security traces data flows and performance calculates numbers, you question the premises that make the code's approach valid.
...
See `_shared-rules.md` for Required Output Format (Toulmin structure). Use `**Category**: assumptions` where `assumptions` is this specialist's category.
