...
- [x] [promoted] **Release Manager Specialist** - Add a 9th specialist with a release engineering cognitive strategy. None of the existing 8 specialists focus on CI/CD pipeline impact, deployment sequencing, rollback safety, migration steps, backward compatibility across rolling deploys, or release cycle considerations. This persona thinks "what happens when this ships?" — tracing changes through build pipelines, checking if new files/dependencies are packaged correctly, asking whether deployment ordering matters, flagging changes that need feature flags or staged rollout, and identifying missing CI workflow updates. Cognitive strategy: release-impact analysis / deployment-path tracing.
