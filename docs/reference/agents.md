...
**Human Control:** Explicit user direction overrides PAW defaults, but not integrity checks or unavailable capabilities. Output capability is discovered from available platform tool operations and every mutation requires exact authorization. Output activities guide mechanics but are not capability gates. GitHub stays pending by default; Azure DevOps/local remain artifact-only only when no executable output action is available.
