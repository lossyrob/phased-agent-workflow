...
| `<context-injection>` | existing | 1 | Dynamic template variables |
| `<responsibility-list>` | existing | 1 | Enumerated responsibilities |
| `<workflow-sequence>` | existing | 1 | Container for ordered workflow steps |
| `<workflow-step>` | existing | 11 | Individual workflow step |
| `<methodology>` | existing | 4 | How agent approaches work |
| `<workflow-adaptation>` | existing | 3 | Mode-specific behavior container |
| `<mode-definition>` | existing | 3 | Specific operating mode |
| `<decision-framework>` | existing | 3 | Criteria/logic for making choices |
| `<detection-criteria>` | existing | 1 | Specific detection patterns |
| `<context-detection>` | existing | 2 | Logic for detecting context/inputs |
| `<scope-boundary>` | existing | 1 | What's in/out of responsibility |
| `<error-handling>` | existing | 2 | Error/edge case handling |
| `<tool-guidance>` | existing | 1 | Tool usage instructions |
| `<example>` | existing | 5 | Illustrative examples |
| `<guardrail>` | existing | 5 | Hard constraints |
| `<handoff-instruction>` | existing | 2 | Instructions for transitioning |
| `<behavioral-directive>` | existing | 1 | Specific behavior instruction |
| `<communication-pattern>` | existing | 1 | User communication guidance |
| `<terminal-state>` | existing | 1 | Workflow completion marker |
| `<posting-rules>` | existing | 1 | GitHub vs local content rules |
| `<artifact-dependency-graph>` | **NEW** | 1 | Artifact prerequisite relationships |
| `<stage-reference>` | **NEW** | 10 | Documentation for specific workflow stage |
| `<two-agent-pattern>` | **NEW** | 1 | Paired agent workflow description |
| `<branching-strategy>` | **NEW** | 2 | Git branch naming/flow conventions |
| `<handoff-mode-definition>` | **NEW** | 3 | Automation level for transitions |
| `<user-scenario>` | **NEW** | 4 | Common usage pattern with guidance |
| `<status-dashboard-structure>` | **NEW** | 1 | Output format for status reports |
| `<discovery-procedure>` | **NEW** | 1 | Steps to discover workflow state |
| `<help-mode>` | **NEW** | 1 | Educational response pattern |
| `<multi-work-management>` | **NEW** | 1 | Managing multiple workflows |
...
<methodology id="implementation-forward-momentum">
...
<methodology id="review-quality-gate">
...
<handoff-mode-definition id="manual">
...
<handoff-mode-definition id="semi-auto">
...
<handoff-mode-definition id="auto">
...
<user-scenario id="mid-workflow-help">
...
<decision-framework id="mode-detection">
...
<decision-framework id="next-step-mapping">
## Next-Step Guidance
...
<guardrail id="github-write-scope">
...
<methodology id="comment-analysis">
