...
Skills follow the [Agent Skills spec](https://agentskills.io/specification). Key principles:

### Description is for Discovery, Body is for Instructions

The `description` frontmatter field tells agents **when** to use the skill. The body content is loaded **after** the skill is activated—it should contain actionable instructions, not explanations of when to load the skill.

**Anti-pattern**: Redundant "when to load" in body
```markdown
---
name: my-skill
description: Handles X for Y workflow.
---
# My Skill
Provides X handling for Y workflow. Load this skill when doing X operations.
```

**Pattern**: Body contains pure instructions
```markdown
---
name: my-skill
description: Handles X for Y workflow. Use when performing X operations.
---
# My Skill
## How to do X
...
If a skill exceeds 500 lines, split detailed content into `references/` files and link to them from SKILL.md.

### Use Forward Slashes in Paths

Always use Unix-style paths (`scripts/helper.py`), never Windows-style (`scripts\helper.py`).
