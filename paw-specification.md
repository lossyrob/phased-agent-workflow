...
Turns an Issue/brief into a **testable specification** (`Spec.md`) and writes `prompts/spec-research.prompt.md` with open factual questions. These questions now cover BOTH:
1. **Internal system behavior** (current capabilities, flows, data, constraints)
2. **External knowledge** (relevant standards, best practices, domain / regulatory constraints, performance or accessibility benchmarks, common architectural patterns) needed to ensure requirements are unambiguous and aligned with industry expectations.

The developer iterates with the agent to clarify and refine the spec, and to run the Spec Research Agent when there are open questions that
require knowledge about the current system **or external context**.
...
It questions the developer to clarify any ambiguities or gaps in the feature description, and iterates with the developer until the spec is clear, complete, and testable.

### Spec Research Agent
Answers the `prompts/spec-research.prompt.md` prompt and produces **factual documentation** in `SpecResearch.md` covering:
1. Internal system behavior (no code line granularity; behavioral & structural facts)
2. External knowledge & best practices (standards, norms, benchmarks, regulatory notes) with cited authoritative sources.

If external search tooling is available it performs targeted queries; otherwise it produces a checklist for user-provided external knowledge or an explicit recorded skip. Skipped external sections are acceptable only if the Spec Agent confirms no critical ambiguity results.

This differs from the Code Research Agent, in that it does not specify file paths / deep implementation, but rather how the system behaves today and what external context should influence *what* is specified (not *how* it will be implemented).

The `prompts/spec-research.prompt.md` is initially generated by the Spec Agent; the developer may refine it (adding or removing internal/external questions) before running research.

The agent creates factual, source-backed documentation—no design, no improvements, no speculation. All external claims must cite a source or be explicitly marked as an assumption awaiting confirmation.
...
* `/docs/agents/<target_branch>/SpecResearch.md` containing both Internal System Behavior and External Knowledge & Best Practices sections (or an explicit note that external research was skipped with listed potential impact areas)
...
- External standards / guidelines / benchmarks relevant to the feature scope (with citations)
- Industry patterns or conventions shaping requirement expectations
...
- **External Web Research**: Querying authoritative sources for standards / best practices; citing each source (title + URL + accessed date)
- **Evidence Citation**: Always backing claims with concrete internal or external evidence
...
- **Is Factual**: Documents only what exists internally + external facts from cited sources
- **Is Behavioral & Contextual**: Focuses on what the system does and relevant external context, not how code works
- **Is Complete**: Answers all internal & external questions from the prompt thoroughly (or explicitly records skipped external items)
- **Is Cited**: Backs external claims with authoritative sources
