...
- **State-echo banner (conditional)**: Emit a single line of the form `[<skill-name>] work-id=<id> | <non-terminal items with status> | Reconciliation=<marker> | Next gate: <N→M or none>` only when (a) `Reconciliation` is not `current`, (b) drift is detected, or (c) the skill is about to cross a stage boundary. Do not emit on the happy path when state is already `current` — the echo is a diagnostic signal, not a per-load receipt.
